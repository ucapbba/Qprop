...
*/
  inline long index(long xindex, long yindex, long zindex) const {
    long result;
...
*/
  inline long index(long r, long l, long m, long n) const {
    long result;
...
*/
  inline long rlmindex(long rindex, long ell, long m) const {
    return (ell*(ell+1)+m)*ngps_x()+rindex;
  };


  inline long nlindex(long nindex, long lindex) const {  
      long n,l;
      long result;
      n=nindex+1;
      l=lindex;
  ...
  inline double r(long index) const {
      return (index+1)*delta_x;
  };
  
  // Attention! This is for TF where we have
  // extra points for the boundaries.
  inline double rho(long index) const {
       return (index-0.5)*delta_x;
  };


  inline double x(long index) const {
      return (index-offse_x+0.5)*delta_x;
  };
  
  inline double y(long index) const {
      return (index-offse_y+0.5)*delta_y;
  };
  
  inline double z(long index) const {
      return (index-offse_z+0.5)*delta_z;
  };

  inline long rindex(double r) const {
      return long( r/delta_x - 0.5);
  };

  inline long rhoindex(double rho) const {
      return long( rho/delta_x + 0.5);
  };

  inline long yindex(double y) const {
