...
*/
void wavefunction::init(long lsize)
{
   wf_dim = lsize;
	  ...
	  for (zindex=0; zindex<g.ngps_z(); zindex++)
	  {
		...
		case 1 :
		  if (ells[zindex]==yindex)
		    {
		      start[index_i]=cplxd(1.0,0.0); //cplxd(-rand()/(RAND_MAX+1.0)+0.5,0.0);
		    }
		  else
		    {
		...
		case 2 :
		  ...
		  switch (zindex)
		    ...
		    case 1: 
                      if (ells[zindex]==yindex)
			{
			  start[index_i]=cplxd(0.0,0.0);//r*exp(-0.5*w*r);
			}
		      else
			{
			  start[index_i]=cplxd(0.0,0.0);
		    ...
		    case 2: 
                      if (ells[zindex]==yindex)
			{
			  start[index_i]=cplxd(0.0,0.0);//r*r*exp(-0.5*w*r);
			}
		      else
			{
			  start[index_i]=cplxd(0.0,0.0);
	  ...
	  for (m=-yindex; m<=yindex; m++)
		...
		case 1 :
		  if ((yindex==(int)(ells[zindex])) && (m==(int)(ms[zindex])))
		    {
		      start[index]=cplxd(1.0,0.0);//cplxd(1.0*rand()/(RAND_MAX+1.0)-0.5,0.0);
		    }
		  else
		    {
...
void wavefunction::init(grid g, FILE* file, int output_of_interest, int iv)
{
  long index;
  double realpart, imagpart;
  long ctrl_id,i;
...
void wavefunction::load(FILE* os, int iv)
{
  double re, im;
  long ctrl_id;
...
// simple case with just one orbital; overloaded below
// This version does not touch zero elements of a wavefunction
void wavefunction::normalize(grid g, int mode)
{
  if (mode==0)
      normalize(g);
  else if (mode==1)
  {
    const double coeff = 1.0/sqrt(norm(g));
...
void wavefunction::subtract(grid g, wavefunction &wf)
{
  cplxd result(0.0,0.0);
  long xindex, yindex, zindex,m;
  long index,indexm,indexmm;
  ...
  switch (g.dimens())
  {
	...
	{
	  index=g.index(xindex,yindex,0);
	  result=result+(conj(wf[index])*start[index])*cplxd(g.delt_x());
	};
      };
      for (xindex=0; xindex<g.ngps_x(); xindex++)
      {
	for (yindex=0; yindex<g.ngps_y(); yindex++)
	{
            ...
            if (WF!=cplxd(0,0))
            {
                start[index] = start[index] - result*WF;
            };
        };
      };
      break;
	...
	for (yindex=0; yindex<g.ngps_y(); yindex++)
	{
            for (m=-yindex; m<yindex+1; m++)
            {
                index=g.index(xindex,yindex,m);
                result=result+(conj(wf[index])*start[index])*cplxd(g.delt_x());
            };
        };
      };
    ...
    default :
      fprintf(stderr,"err: propagation mode unknown or not implemented (g.dimens())\n");
      exit(-52);
  }
}


void wavefunction::extract_ell_m(grid g, long ell, long m0)
{
  long xindex, yindex, m;
  long index;
  switch (g.dimens())
  {
  case 34 :
    for (yindex=labs(m0); yindex<g.ngps_y(); yindex++)
    {
        if (yindex!=ell)
        {
            for (xindex=0; xindex<g.ngps_x(); xindex++)
            {
                index=g.index(xindex,yindex,m0);
                start[index] = cplxd(0.0,0.0);
            };
        };
    };
    break;
    case 44 :
    for (yindex=m0*m0; yindex<g.ngps_y(); yindex++)
    {
        for (m=-yindex; m<yindex+1; m++)
        {
            if ( !((yindex==ell)&&(m==m0)) )
    ...
    break;
  default :
      fprintf(stderr,"err: wrong propagation mode (g.dimens())\n");
      exit(-52);
  };
};
...
cplxd wavefunction::expect_z(grid g, int m)
{
    cplxd result(0.0,0.0);
    long xindex, ell, zindex;
    long index, ell_m, ell_p;
    double r;
    double dr = g.delt_x();
    ...
    case 34 : 
        for (zindex=0; zindex<g.ngps_z(); zindex++)
        {
            ...
            { 
                r=g.r(xindex);
                ell=0;
                index=g.index(xindex,ell,zindex);
                ell_p=g.index(xindex,ell+1,zindex);
                
                result=result+(cplxd)(r)*start[index]
                *conj(start[ell_p])*(cplxd)sqrt((ell+1.0)*(ell+1.0)-m*m)/sqrt((2.0*ell+1.0)*(2.0*ell+3.0));

                for (ell=1; ell<g.ngps_y()-1; ell++)
                {
                    index=g.index(xindex,ell,zindex);
                    ell_p=g.index(xindex,ell+1,zindex);
                    ell_m=g.index(xindex,ell-1,zindex);
                    
                    result=result+(cplxd)(r)*start[index]
                    *(  conj(start[ell_p])*(cplxd)sqrt((ell+1.0)*(ell+1.0)-m*m)/sqrt((2.0*ell+1.0)*(2.0*ell+3.0))
                       +conj(start[ell_m])*(cplxd)sqrt(ell*ell-m*m)/sqrt((2.0*ell-1.0)*(2.0*ell+1.0)) );
  ...
  switch (g.dimens())
  {
	  ...
	  { 
		  ...
		  if (g.ngps_y() > 1)  {
		      ...
		      ell=0;
		      index=g.index(xindex,ell,zindex);
		      index_yp=g.index(xindex,ell+1,zindex);
		      
		      result=result+conj(start[index_yp])*r*start[index]*sqrt((ell+1.0)*(ell+1)-ms[zindex]*ms[zindex])/sqrt((2.0*ell+1.0)*(2.0*ell+3.0))*g.delt_x()*degeneracies[zindex];
		      
		      for (ell=1; ell<g.ngps_y()-1; ell++)
		      {
			  index=g.index(xindex,ell,zindex);
			  index_yp=g.index(xindex,ell+1,zindex);
			  index_ym=g.index(xindex,ell-1,zindex);
			  
			  result=result+g.delt_x()*degeneracies[zindex]*r*start[index]
			      *((conj(start[index_yp]))*(sqrt((ell+1.0)*(ell+1)-ms[zindex]*ms[zindex]))/sqrt((2.0*ell+1.0)*(2.0*ell+3.0))
				+(conj(start[index_ym]))*(sqrt(ell*ell-ms[zindex]*ms[zindex]))/sqrt((2.0*ell-1.0)*(2.0*ell+1.0)));
		    ...
		    result=result+(g.delt_x()*degeneracies[zindex]*r)*start[index]*conj(start[index_ym])*(sqrt(ell*ell-ms[zindex]*ms[zindex]))/sqrt((2.0*ell-1.0)*(2.0*ell+1.0));
		  };    
	      };
	  };
...
cplxd wavefunction::accel_z(grid g, int m, hamop hamil, double time, int me, double deltat)
{
    cplxd result(0.0,0.0);
    long xindex, ell, zindex;
    long index, ell_m, ell_p;
    ...
    double r;
    double dr = g.delt_x();
    double dt = 0.5*deltat;
    cplxd Ez  = (hamil.vecpot_z(time+dt,me) - hamil.vecpot_z(time-dt,me))/deltat;
    cplxd dU_dr;
    ...
    case 34 : 
        for (zindex=0; zindex<g.ngps_z(); zindex++)
        {
            for (xindex=0; xindex<Nx; xindex++)
            {
                ...
                if (xindex>0)
                {
                        dU_dr = (hamil.scalarpot(r+dr,0,0, time,me)-hamil.scalarpot(r-dr,0,0, time,me))*0.5/dr;
                } else
                {
                    dU_dr = (hamil.scalarpot(r+dr,0,0, time,me)-hamil.scalarpot(r,0,0, time,me))/dr;
                };
                //dU_dr = (1.0+a*exp(-b*r))/(r*r)+a*b*exp(-b*r)/r; // <-- Here the explicit form for potential U(r) = -(1+a*exp(-br))/r
                ...
                result=result+(cplxd)(-dU_dr)*start[index]*
                       ( conj(start[ell_p])*(cplxd)sqrt((ell+1.0)*(ell+1.0)-m*m)/sqrt((2.0*ell+1.0)*(2.0*ell+3.0)) );
        ...
        result = result*(cplxd)dr - (cplxd)Ez;
        break;
    };
...
cplxd wavefunction::expect_cycl_pol_plus(grid g)
{
    ...
    case 44 :
        for (xindex=0; xindex<g.ngps_x(); xindex++)
        {
            ...
            result=result+(cplxd)(r)*conj(start[index])*
                   ( -start[index_lpmm]*sqrt((ell+1.0)*(ell-m+1.0)*(ell-m+2.0)/((2.0*ell+1)*(2.0*ell+2)*(2.0*ell+3))) );
            ...
            for (ell=1; ell<g.ngps_y()-1; ell++)
            {
                for (m=-ell; m<=ell; m++)
                {
                    ...
                    if (abs(m-1) <= ell-1)
                    {
                        ...
                        result=result+(cplxd)(r)*conj(start[index])*
                               ( start[index_lmmm]*sqrt((ell+m-1.0)*(ell+m)/(2.0*(2.0*ell-1)*(2.0*ell+1))) );
                    };
                    if (abs(m-1) <= ell+1)
                    {
                        index_lpmm=g.rlmindex(xindex,ell+1,m-1);
...
cplxd wavefunction::accel_cycl_pol_plus(grid g, hamop hamil, double time, int me, double deltat)
{
    cplxd result(0.0,0.0);
    long xindex, ell, m;
    long index, index_lmmm, index_lpmm;
    ...
    double dt =  0.5*deltat;
    cplxd imagi(0.0,1.0);
    cplxd A_p =  hamil.vecpot_x(time+dt,me) + imagi*hamil.vecpot_y(time+dt,me);
    cplxd A_m =  hamil.vecpot_x(time-dt,me) + imagi*hamil.vecpot_y(time-dt,me);
    cplxd E   = -(A_p-A_m)/deltat;
    cplxd dU_dr;
    ...
    case 44 :
        for (xindex=0; xindex<Nx; xindex++)
        {
            ...
            result=result+(cplxd)(-dU_dr)*conj(start[index])*
                   ( -start[index_lpmm]*sqrt((ell+1.0)*(ell-m+1.0)*(ell-m+2.0)/((2.0*ell+1)*(2.0*ell+2)*(2.0*ell+3))) );
        ...
        result=-SQRTT*(cplxd)dr*result + (cplxd)E;
        break;
    };
