...
int v = 0; // verbosity. 0 - silent, 1 - print out
...
void parameterListe::setDouble(string name, double value) {
  ...
  it = find_if(_param_list.begin(), _param_list.end(), mycmp); 
  if (it == _param_list.end()) {
    std::cerr << "Parameter " << name << "  is not found  in the parameterListe::getDouble" << std::endl;
    exit(-1);
  }
  else if (it->_type != "double") {
    std::cerr << "Wrong type in the parameterListe::getDouble" << std::endl;
    exit(-1);
  }  
  else {
    stringstream bla;
...
long parameterListe::getLong(string name)  const {
  ...
  if (it == _param_list.end())
    {
      std::cerr << "Parameter " << name << "  is not found  in the parameterListe::getLong" << std::endl;
      exit(-1);
    }
  else if (it->_type != "long")
    {
      std::cerr << "Wrong type in the parameterListe::getLong" << std::endl;
      exit(-1);
    }  
  else
    {
...
string parameterListe::getString(string name)  const {
  ...
  it = find_if(_param_list.begin(), _param_list.end(), mycmp); 
  if (it == _param_list.end()) {
    std::cerr << "Parameter " << name << "  is not found  in the parameterListe::getString" << std::endl;
    exit(-1);
  }
  else if (it->_type != "string") {
    std::cerr << "Wrong type in the parameterListe::getString" << std::endl;
    exit(-1);
  }  
  else {
    stringstream bla(it->_value);
...
bool parameterListe::getBool(string name)  const {
  ...
  it = find_if(_param_list.begin(), _param_list.end(), mycmp); 
  if (it == _param_list.end()) {
    std::cerr << "Parameter " << name << "  is not found  in the parameterListe::getBool" << std::endl;
    exit(-1);
  }
  else if (it->_type != "bool") {
    std::cerr << "Wrong type in the parameterListe::getBool" << std::endl;
    exit(-1);
  }  
  else {
    stringstream bla(it->_value);
