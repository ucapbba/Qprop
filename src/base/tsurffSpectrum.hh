...
inline string fixed_length_number(long Nzeros, long n)
{
    string STR_0 = to_string(Nzeros);
    string STR   = to_string(n);
    return string(STR_0.length() - STR.length(), '0') + STR;
};
...
public:
    ...
    : psi( string("./dat/")+filename+string("_psi.raw"), std::ios::binary ), dpsi_dr( string("./dat/")+filename+string("_dpsidr.raw"), std::ios::binary ) {
        qprop_dim = param_ini.getLong("qprop-dim");
        delta_r = param_ini.getDouble("delta-r");
        R_tsurff = param_prop.getDouble("R-tsurff");
...
class tsurffSpectrum {
	...
	cplxd_ptr partial_amplitude_isurfv;
...
public:
    ...
    : thetas_surff(101, 0.0), phis_surff(101, 0.0), k_values(100, 0.0), psi( string("./dat/")+filename+string("_psi.raw"), std::ios::binary ), dpsi_dr( string("./dat/")+filename+string("_dpsidr.raw"), std::ios::binary ), vecpot_x(vecpotx), vecpot_y(vecpoty), vecpot_z(vecpotz) {
        ...
        tsurff_method   = param_tsurff.getLong("tsurff-version");
        additional_time = param_prop.getDouble("additional-time");
        ...
        const double time_surff = param_prop.getDouble("R-tsurff")/param_tsurff.getDouble("k-min-tsurff");
    ...
    */
    ...
    void print_partial_amplitudes() {
        if (expansion_scheme==2) {
            ofstream tsurff_partial_dat("./dat/tsurff_partial_"+fixed_length_number(num_proc, i_proc)+".dat");
			ofstream isurfv_partial_dat("./dat/isurfv_partial_"+fixed_length_number(num_proc, i_proc)+".dat");
			double norm_a_ell_k;
			double norm_a_ell_k_isurfv;
            if (qprop_dim==34 && expansion_scheme==2) {
                for (long i_k=0; i_k<num_k_proc; i_k++) {
                    double total_spectrum_k(0.0);
					double total_spectrum_k_isurfv(0.0);
                    const double k=k_values[i_k];
                    tsurff_partial_dat << k*k*0.5 << " " << k;
					if (tsurff_method == 2)
					{
						isurfv_partial_dat << k*k*0.5 << " " << k;
					};
                    for (long i_ell=0; i_ell<ell_grid_size; i_ell++) {
                        const long ind_ampl=index_ell_k(i_ell, i_k);
                        tsurff_partial_dat << " " << real(partial_amplitude[ind_ampl]) << " " << imag(partial_amplitude[ind_ampl]);
                        norm_a_ell_k = norm(partial_amplitude[ind_ampl]);
                        total_spectrum_k += norm_a_ell_k;
						// tsurff_partial_dat << " " << norm_a_ell_m_k*k;
						if (tsurff_method == 2)
						{
							isurfv_partial_dat << " " << real(partial_amplitude_isurfv[ind_ampl]) << " " << imag(partial_amplitude_isurfv[ind_ampl]);
							norm_a_ell_k_isurfv = norm(partial_amplitude_isurfv[ind_ampl]);
							total_spectrum_k_isurfv += norm_a_ell_k_isurfv;
							// isurfv_partial_dat << " " << norm_a_ell_m_k_isurfv*k;
						};
                    };
                    tsurff_partial_dat << " " << total_spectrum_k*k << endl;
					if (tsurff_method == 2)
					{
						isurfv_partial_dat << " " << total_spectrum_k_isurfv*k << endl;
					};
                };
            }
                ...
                for (long i_k=0; i_k<num_k_proc; i_k++) {
                    ...
                    for (long i_ell=0; i_ell<ell_grid_size; i_ell++) {
                        for (long i_m=-(i_ell); i_m<(i_ell+1); i_m++) {
							...
							{
								isurfv_partial_dat << " " << real(partial_amplitude_isurfv[ind_ampl]) << " " << imag(partial_amplitude_isurfv[ind_ampl]);
								const double norm_a_ell_m_k_isurfv=norm(partial_amplitude_isurfv[ind_ampl]);
								total_spectrum_k_isurfv += norm_a_ell_m_k_isurfv;
								// isurfv_partial_dat << " " << norm_a_ell_m_k_isurfv*k;
							};
                        };
                    };
...
//#endif
        ...
        if (delta_k_scheme==1) {
            ...
            for (long i_k=i_k_min; i_k<i_k_max; i_k++) {
                k_values[i_k-i_k_min]=double(i_k)*delta_k_surff;
            };
        }
        else if (delta_k_scheme==2) {
            const double delta_k_surff=pow2(k_max_surff)/double(num_k_surff);
            for (long i_k=i_k_min; i_k<i_k_max; i_k++) {
                k_values[i_k-i_k_min]=sqrt(double(i_k)*delta_k_surff);
            };
        }
        else {
        ...
        // If N_{\theta}=1, angle 0.5\pi is taken
        if (num_theta_surff==1)
            thetas_surff[0]=0.5*M_PI;

        if (num_theta_surff>1)
        {
            // it is more convenient if number of thetas is odd
            if (num_theta_surff%2==0) 
                num_theta_surff++; 
            if (num_theta_surff>101) 
                thetas_surff.resize(num_theta_surff);
            // min and max \theta are always be 0 and \pi
            const double delta_theta = M_PI/double(num_theta_surff-1);
            for (long i_theta=0; i_theta<num_theta_surff; i_theta++) 
            {
                thetas_surff[i_theta]=(i_theta)*delta_theta;
            };
        };
        ...
        */
        if (num_phi_surff>101) 
            phis_surff.resize(num_phi_surff);
        
        const double delta_phi=2.0*M_PI/(double(num_phi_surff)-1.0);
        for (long i_phi=0; i_phi<num_phi_surff; i_phi++) {
            phis_surff[i_phi]=(i_phi)*delta_phi;
        };
    };
    ...
    void prep_space() {
        ...
        */
        const long num_psi_size_surff=(expansion_scheme==1)?(ell_m_grid_size*num_theta_surff*num_phi_surff*num_k_proc):ell_m_grid_size*ell_m_grid_size*num_k_proc;
        ...
        if (expansion_scheme==2)
		{
			...
			if (tsurff_method == 2)
			{
				partial_amplitude_isurfv.reset(new cplxd[ell_m_grid_size*num_k_proc]);
			};
		};
        ...
        if (i_proc==0)
            cout << "saving the time integrals will take " << ceil(((qprop_dim==44)?4:3)*16.0*num_psi_size_surff/1048576.0) << "x" << num_proc << " MB of space" << endl;
    };
...
//                    hanning = exp(-0.01*(time-pulse_duration));
         } else if (tsurff_method == 2)
         {
             hanning = 1.0;
         };
    ...
    /// Side effects: writes to file tsurff_polari_proc.dat \n
    ...
    */
    void eval_spectrum_34_theta_phi() 
    {
        ...
        if (tsurff_method == 2) 
        {
            ...
            long i_k_min = num_k_proc*i_proc;
//             cout << i_k_min << " lines are ignored while reading the file " << endl;
            for (long lenergy=0; lenergy<i_k_min; lenergy++) // ignoring first i_k_min lines in the data file
            {
                fscanf_status=fscanf(file, "%lf %lf", &energy, &k);
                for (long i_ell=0; i_ell<ell_grid_size; i_ell++)
                {
                    fscanf_status=fscanf(file, "%lf %lf %lf %lf", &psi_re, &psi_im, &d_psi_dr_re, &d_psi_dr_im);
                };
            };
        };
       ...
       // ofstream tsurff_polar_dat("./dat/tsurff_polar_"+fixed_length_number(num_proc, i_proc)+".dat");
        ...
        for (long i_k=0; i_k<num_k_proc; i_k++) 
        {
            ...
            if (tsurff_method == 2) 
            {
                long fscanf_status;
                double psi_re, psi_im,d_psi_dr_re, d_psi_dr_im;
                fscanf_status=fscanf(file, "%lf %lf", &energy, &k); // read   Ek  k
                for (long i_ell=0; i_ell<ell_grid_size; i_ell++)
                {
                    fscanf_status=fscanf(file, "%lf %lf %lf %lf", &psi_re, &psi_im, &d_psi_dr_re, &d_psi_dr_im); // read psi_re   psi_im   dpsi_dr_re   dpsi_dr_im
                    psi_R_inf[i_ell]       = cplxd(psi_re, psi_im);
                    psi_deriv_R_inf[i_ell] = cplxd(d_psi_dr_re, d_psi_dr_im);
                };
            } else {
                k=k_values[i_k];
                energy = 0.5*k*k;
            ...
            for (long i_theta=0; i_theta<num_theta_surff; i_theta++) 
            {
                ...
                const double cos_theta=cos(thetas_surff[i_theta]);
                cplxd b_k(0.0, 0.0);
                cplxd b_k_analytic(0.0, 0.0);
                for (long ell=labs(initial_m); ell<ell_grid_size; ell++) 
                {
                    // $c_{\ell, m}=\sqrt{\frac{(\ell+1)^2-m^2}{(2\ell+1)(2\ell+3)}}$
                    double c_ell_m=sqrt(pow2(ell+1)-pow2(initial_m))*sqrt(1./double((2*ell+1)*(2*ell+3)));
                    // \frac{d}{dr} f_{\ell}(kr)&=-kf_{\ell+1}(kr) + \frac{\ell}{r}f_{\ell}(kr))
                    const double j_lkr_deriv=-k*j_lkr[ell+1] + double(ell)/R_tsurff*j_lkr[ell];
                    const long ind_surff=index_ell_theta_k(ell, i_theta, i_k);

                    // spherical harmonics for \ell and \ell+1  
                    const cplxd Ylm=Ylm_prefactor(initial_m)*gsl_sf_legendre_sphPlm (ell, labs(initial_m), cos_theta);
                    const cplxd Ylp1m=Ylm_prefactor(initial_m)*gsl_sf_legendre_sphPlm (ell+1, labs(initial_m), cos_theta);
                    // contributions including the vector potential A
                    // \psi_{\ell} and Y_{\ell+1} are coupled here
                    const cplxd b_kl_Ylm_1=cplxd(0., -1.0)*cplxd(0., 2.0)*c_ell_m*j_lkr[ell+1]*Ylp1m*psi_A_surff[ind_surff];
                    // \psi_{\ell+1} and Y_{\ell} are coupled here (for \ell=N_{\ell}-1 this is set to zero)
                    const cplxd b_kl_psi_1=((ell+1)<ell_grid_size)?(cplxd(0., 2.0)*c_ell_m*j_lkr[ell]*Ylm*psi_A_surff[ind_surff+1]):0.0;
                    const cplxd b_kl =  b_kl_Ylm_1
                                      + b_kl_psi_1
                                      + (   j_lkr[ell]*(psi_deriv_surff[ind_surff]-psi_surff[ind_surff]/R_tsurff)
                                          - j_lkr_deriv * psi_surff[ind_surff]
                                         )*Ylm;
                                         
                    b_k += pow_neg_i(ell+1)*b_kl;
                
                    if (tsurff_method == 2) 
                    {
                        const cplxd b_kl_analytic = (  j_lkr[ell]*(psi_deriv_R_inf[ell]-psi_R_inf[ell]/R_tsurff)
                                                     - j_lkr_deriv*psi_R_inf[ell])*Ylm;
                                                     
                        b_k_analytic += pow_neg_i(ell+1)*b_kl_analytic;
                    };
                    
                }; // ell-loop
                b_k*=R_tsurff/sqrt(2.*M_PI)*delta_t;
				tsurff_polar_dat << energy << " " << k << " " << theta << " " << "0" << " " << norm(b_k)*k << " " << real(b_k) << " " << imag(b_k) << endl;
				
                if (tsurff_method == 2) 
                {
                    b_k_analytic *= R_tsurff/sqrt(2.*M_PI)*cplxd(0.0, 1.0)*exp(cplxd(0.0, 1.0)*energy*duration);
                    b_k += b_k_analytic;
					isurfv_polar_dat << energy << " " << k << " " << theta << " " << "0" << " " << norm(b_k)*k << " " << real(b_k) << " " << imag(b_k) << endl;
                };
    ...
    */
    void eval_spectrum_44_theta_phi() {
        ...
        if (tsurff_method == 2) 
        {
            ...
            long i_k_min = num_k_proc*i_proc;
//             cout << i_k_min << " lines are ignored while reading the file " << endl;
            for (long i_k=0; i_k<i_k_min; i_k++) // ignoring first i_k_min lines in the data file
            {
                fscanf_status=fscanf(file, "%lf %lf", &energy, &k);
                for (long i_ell=0; i_ell<ell_grid_size; i_ell++)
                {
        ...
        for (long i_k=0; i_k<num_k_proc; i_k++) 
        {
            ...
            if (tsurff_method == 2) 
            {
                ...
                for (long i_ell=0; i_ell<ell_grid_size; i_ell++)
                {
                    for (long i_m=-long(i_ell); i_m<long(i_ell+1); i_m++) 
                    {
                        long lmindex = i_ell*(i_ell+1) + i_m;
                        fscanf_status=fscanf(file, "%lf %lf %lf %lf", &psi_re, &psi_im, &d_psi_dr_re, &d_psi_dr_im); // read psi_re   psi_im   dpsi_dr_re   dpsi_dr_im
                        psi_R_inf[lmindex]       = cplxd(psi_re, psi_im);
                        psi_deriv_R_inf[lmindex] = cplxd(d_psi_dr_re, d_psi_dr_im);
                    };
                };
            } 
            else 
                ...
                for (long i_phi=0; i_phi<num_phi_surff; i_phi++) 
                {
                    ...
                    for (long i_ell=0; i_ell<ell_grid_size; i_ell++) 
                    {
                        ...
                        for (long i_m=-long(i_ell); i_m<long(i_ell+1); i_m++) 
                        {
                            ...
                            {
                                long lmindex = i_ell*(i_ell+1) + i_m;
                                b_k_analytic += pow_neg_i(i_ell+1)*( j_lkr[i_ell]*(psi_deriv_R_inf[lmindex]-1.0/R_tsurff*psi_R_inf[lmindex])
                                                                   -j_lkr_deriv*psi_R_inf[lmindex])*Ylm;
                            };
                            
                        };
                    };
                    b_k *= R_tsurff/sqrt(2.*M_PI)*delta_t;
					tsurff_polar_dat << energy << " " << k << " " << theta << " " << phi << " " << norm(b_k)*k << " " << real(b_k) << " " << imag(b_k) << endl;
                    if (tsurff_method == 2) 
                    {
                        b_k_analytic *= R_tsurff/sqrt(2.*M_PI)*cplxd(0.0,1.0)*exp(cplxd(0.0,1.0)*energy*duration);
                        b_k += b_k_analytic;
						isurfv_polar_dat << energy << " " << k << " " << theta << " " << phi << " " << norm(b_k)*k << " " << real(b_k) << " " << imag(b_k) << endl;
                    };
    ...
    */
    void eval_partial_spectra_34_ell1_ell2() 
    {
        ...
        {
            if ((i_proc==0)&&(i_ell%100==0))
                cout << i_ell << " of " << ell_grid_size << " complete." << endl;
            ...
            for (long i_k=0; i_k<num_k_proc; i_k++) 
            {
				...
				if (tsurff_method == 2)
				{
					partial_amplitude_isurfv[index_ell_k(i_ell, i_k)] = partial_amplitude[index_ell_k(i_ell, i_k)];
				};
            };
        };
        if (i_proc==0)
                cout << "All ell complete." << endl;
        if (tsurff_method == 2)
        {
            if (i_proc==0)
                cout << "Applying the i-SURFV part." << endl;
            FILE* file=fopen("./dat/isurfv.dat", "r");
            double energy,k;
            long fscanf_status;
            double psi_re, psi_im,d_psi_dr_re, d_psi_dr_im;
            long i_k_min = num_k_proc*i_proc;
            // cout << i_k_min << " lines are ignored while reading the file " << endl;
            for (long lenergy=0; lenergy<i_k_min; lenergy++) // Ignoring first i_k_min lines
            {
                fscanf_status=fscanf(file, "%lf %lf", &energy, &k);
                for (long ell=0; ell<ell_grid_size; ell++)
                {
                    fscanf_status=fscanf(file, "%lf %lf %lf %lf", &psi_re, &psi_im, &d_psi_dr_re, &d_psi_dr_im);
                };
            };
            cplxd psi_R_inf, psi_deriv_R_inf;
            for (long i_k=0; i_k<num_k_proc; i_k++) 
            {
                if ((i_proc==0)&&(i_k%100==0))
                    cout << i_k << " k values of " << num_k_proc << " complete. " << endl;
                fscanf_status=fscanf(file, "%lf %lf", &energy, &k); // Reading Ek k
                k=k_values[i_k];
                fill_bessel_array(k, R_tsurff, j_lkr);
                for (long i_ell=labs(initial_m); i_ell<ell_grid_size; i_ell++) 
                {
                    fscanf_status=fscanf(file, "%lf %lf %lf %lf", &psi_re, &psi_im, &d_psi_dr_re, &d_psi_dr_im); // read psi_re   psi_im   dpsi_dr_re   dpsi_dr_im
                    psi_R_inf       = cplxd(psi_re,psi_im);
                    psi_deriv_R_inf = cplxd(d_psi_dr_re,d_psi_dr_im);
                    const double j_lkr_deriv=-k*j_lkr[i_ell+1] + double(i_ell)/R_tsurff*j_lkr[i_ell];
                    // Calculating R/sqrt*j/(jell_kr*(d_dr-(ell+1)/R+jellp1_kr*k)*1/(H-Ek)*psi_ell(R)
                    const cplxd b_kl_analytic = pow_neg_i(i_ell+1)*( j_lkr[i_ell]*(psi_deriv_R_inf-psi_R_inf/R_tsurff) - j_lkr_deriv*psi_R_inf );
                    const cplxd partial_amplitude_analytic = b_kl_analytic*cplxd(0.0,1.0)*R_tsurff/sqrt(2.*M_PI)*exp(cplxd(0.0,1.0)*energy*duration);
                    partial_amplitude_isurfv[index_ell_k(i_ell, i_k)] += partial_amplitude_analytic;
                };
            };
        };
        if (i_proc==0)
            cout << "All partial amplitudes complete." << endl;
    };
    ...
    */
        ...
        for (long i_k=0; i_k<num_k_proc; i_k++) 
        {
            ...
            for (long i_theta=0; i_theta<num_theta_surff; i_theta++) 
            {
                ...
                for (long i_ell=labs(initial_m); i_ell<ell_grid_size; i_ell++) 
                {
					...
					if (tsurff_method == 2)
					{
						b_k_isurfv+=partial_amplitude_isurfv[index_ell_k(i_ell, i_k)]*Ylm;
					};
                };
                tsurff_polar_dat << k*k*0.5 << " " << k << " " << theta << " " << "0" << " " << norm(b_k)*k << " " << real(b_k) << " " << imag(b_k) << endl;
				if (tsurff_method == 2)
				{
					isurfv_polar_dat << k*k*0.5 << " " << k << " " << theta << " " << "0" << " " << norm(b_k_isurfv)*k << " " << real(b_k_isurfv) << " " << imag(b_k_isurfv) << endl;
				};
            };
            // Line break for every new theta
            tsurff_polar_dat << endl;
    ...
    {
        const long ell_grid_size_2=ell_grid_size*ell_grid_size;
        return  (i_ell1*(i_ell1+1)+i_m1) + i_ell2*ell_grid_size_2;
    };
...
#define TESTS
    ...
    */
    void eval_partial_spectra_44_ell1_ell2() 
    {
        ...
        {
            if ((i_proc==0)&&(i_ell%100==0))
                cout << i_ell << " ell of " << ell_grid_size << " complete." << endl;
            for (long i_m=-(i_ell); i_m<(i_ell+1); i_m++) 
            {
                ...
                for (long i_k=0; i_k<num_k_proc; i_k++) 
                {
                        ...
                        for (long i_m1=-(i_ell1); i_m1<(i_ell1+1); i_m1++) 
                        {
                            ...
                            for (long i_ell2=0; i_ell2<ell_grid_size; i_ell2++) 
                            {
                                ...
                                if (w_ellmell1m1ell2 != 0) 
                                {
                                    ...
                                    const long N_ell = ell_grid_size;
                                    const cplxd long_joint_factor = w_ellmell1m1ell2*pow_neg_1(i_m1+i_m2)*pow_i(i_ell2-i_ell1)*j_lkr[i_ell1];
                                    if (i_ell1>0 && i_m1>=-long(i_ell1)+2) 
                                    {
                                        ...
                                        b_klm_sum -= long_joint_factor*psi_A_cc_surff[ind_surff_mm1_ellm1]*clebsch_mm1_ellm1(i_ell1, i_m1);
                                    };
                                    if (i_ell1>0 && i_m1<=long(i_ell1)-2) 
                                    {
                                        const long ind_surff_mp1_ellm1 = index_ell1_m1_ell2_m2_k(i_m1+1, long(i_ell1)-1, i_ell2, i_m2, i_k);
                                        b_klm_sum += long_joint_factor*psi_A_surff[ind_surff_mp1_ellm1]*clebsch_mp1_ellm1(i_ell1, i_m1);
                                    };
                                    if (i_ell1<(N_ell-1)) 
                                    {
                                        const long ind_surff_mm1_ellp1 = index_ell1_m1_ell2_m2_k(i_m1-1, i_ell1+1, i_ell2, i_m2, i_k);
                                        b_klm_sum += long_joint_factor*psi_A_cc_surff[ind_surff_mm1_ellp1]*clebsch_mm1_ellp1(i_ell1, i_m1);
                                        const long ind_surff_mp1_ellp1 = index_ell1_m1_ell2_m2_k(i_m1+1, i_ell1+1, i_ell2, i_m2, i_k);
                                        b_klm_sum -= long_joint_factor*psi_A_surff[ind_surff_mp1_ellp1]*clebsch_mp1_ellp1(i_ell1, i_m1);
                                    };
					...
					if (tsurff_method == 2)
					{
						partial_amplitude_isurfv[index_ell_m_k(i_ell, i_m, i_k)] = partial_amplitude[index_ell_m_k(i_ell, i_m, i_k)];
					};
                };
            };
        };
        ...
        if (tsurff_method == 2)
        {
            ...
            long i_k_min = num_k_proc*i_proc;
            // Ignoring first i_k_min lines
            for (long lenergy=0; lenergy<i_k_min; lenergy++) // Missing a line by reading it ...
            {
                fscanf_status=fscanf(file, "%lf %lf", &energy, &k);
                for (long ell=0; ell<ell_grid_size; ell++)
                {
                    for (long i_m=-(ell); i_m<(ell+1); i_m++)
                    {
                        fscanf_status=fscanf(file, "%lf %lf %lf %lf", &psi_re, &psi_im, &d_psi_dr_re, &d_psi_dr_im); // ... till the end .
                    };
                };
            };
            for (long i_k=0; i_k<num_k_proc; i_k++) 
            {
                ...
                for (long ell=0; ell<ell_grid_size; ell++)
                {
                    ...
                    for (long i_m=-(ell); i_m<(ell+1); i_m++) 
                    {
                        long lmindex = ell*(ell+1) + i_m;
                        fscanf_status=fscanf(file, "%lf %lf %lf %lf", &psi_re, &psi_im, &d_psi_dr_re, &d_psi_dr_im); // Read psi_re   psi_im   dpsi_dr_re   dpsi_dr_im
                        psi_R_inf       = cplxd(psi_re, psi_im);
                        psi_deriv_R_inf = cplxd(d_psi_dr_re, d_psi_dr_im);
                        //Calculating R/sqrt*j/(jell_kr*(d_dr-(ell+1)/R+jellp1_kr*k)*1/(H-Ek)*psell(R)
                        const cplxd partial_amplitude_analytic = R_tsurff/sqrt(2.*M_PI)*pow_neg_i(ell+1)*
                                        (  j_lkr[ell]*(psi_deriv_R_inf-psi_R_inf/R_tsurff)
                                         - j_lkr_deriv*psi_R_inf)*cplxd(0.0,1.0)*exp(cplxd(0.0,1.0)*energy*duration);
                        partial_amplitude_isurfv[index_ell_m_k(ell, i_m, i_k)] += partial_amplitude_analytic;
                    };                    
                }; 
            };
        };
    ...
    */
    ...
    {
        if (i_proc==0)
            cout << "Obtaining angle-resolved spetrum." << endl;
        ofstream tsurff_polar_dat("./dat/tsurff_polar_"+fixed_length_number(num_proc, i_proc)+".dat");  
        tsurff_polar_dat.precision(17);
		ofstream isurfv_polar_dat("./dat/isurfv_polar_"+fixed_length_number(num_proc, i_proc)+".dat");  
        isurfv_polar_dat.precision(17);
        ...
        {
            if ((i_proc==0)&&(i_k%100==0))
                cout << i_k << " energies of " << num_k_proc << " complete. " << endl;
            const double k=k_values[i_k];
            for (long i_theta=0; i_theta<num_theta_surff; i_theta++) 
            {
                const double theta=thetas_surff[i_theta];
                ...
                for (long i_phi=0; i_phi<num_phi_surff; i_phi++) 
                {
                    ...
                    for (long i_ell=0; i_ell<ell_grid_size; i_ell++) 
                    {
                        for (long i_m=-(i_ell); i_m<(i_ell+1); i_m++) 
                        {
							...
							if (tsurff_method == 2)
							{
								b_k_isurfv += partial_amplitude_isurfv[index_ell_m_k(i_ell, i_m, i_k)]*Ylm;
							};
                        };
                    };
                    tsurff_polar_dat << k*k*0.5 << " " << k << " " << theta << " " << phi << " " << norm(b_k)*k << " " << real(b_k) << " " << imag(b_k) << endl;
					if (tsurff_method == 2)
					{
						isurfv_polar_dat << k*k*0.5 << " " << k << " " << theta << " " << phi << " " << norm(b_k_isurfv)*k << " " << real(b_k_isurfv) << " " << imag(b_k_isurfv) << endl;
					};
                };
                // line break for every new phi
                tsurff_polar_dat << endl;
...
public:
    ...
    void print_Plms() {
        if (i_proc==0) {
            ofstream debug_Plm_dat("./dat/debug_Plm.dat");
    ...
    void print_bessel() {
        if (i_proc==0) {
            ofstream debug_bessel_dat("./dat/debug_bessel.dat");
    ...
    void print_int_dt_psi(long i_theta=0) {
        if (expansion_scheme==1) {
            ofstream tsurff_input_dat("./dat/tsurff_int_dt_psi-"+to_string(i_proc)+".dat");
            const double cos_theta=cos(thetas_surff[i_theta]);
            for (long i_k=0; i_k<num_k_proc; i_k++) {
                const double k=k_values[i_k];
                for (long i_ell=0; i_ell<ell_grid_size; i_ell++) {
    ...
    void print_wigner() {
        if (i_proc==0) {
            ofstream debug_wigner_dat("./dat/debug_wigner.dat");
            if (qprop_dim==34) {
                double_ptr wigner_pre(new double[ell_grid_size*ell_grid_size]);
                for (long i_ell=0; i_ell<ell_grid_size; i_ell++) {
